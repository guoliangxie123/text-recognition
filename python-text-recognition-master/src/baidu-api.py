...
def getOcr(img_path: object) -> object:
    ocr = CnOcr(rec_root = "./cnocr",det_root="./cnstd")  # 所有参数都使用默认值
...
class MyApp(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self):
        ...
        super().__init__()
        self.initUI()  # 调用自定义的UI初始化函数initUI()D
        self.status = 1  # 状态变量，如果是打开图片来转换的，设置status为True，以区分截图时调用的图片转换函数
    ...
    def initUI(self):
        ...
        self.setupUi(self)
        self.setWindowTitle("先导智能OCR")
        self.setWindowIcon(QIcon("../icons/eye.png"))  # 设置图标，linux下只有任务栏会显示图标
        self.plainTextEdit.setPlainText('图片识别区')
        # self.plainTextEdit.setPlainText('图片识别区')

        # self.plainTextEdit.setPlainText(self.convertButton_callback())
    ...
    def initButton(self):
        ...
        # self.btnBrowse.clicked.connect(self.convertButton_callback)  # 按下按钮调用回调函数
        ...
        # # self.btnScreen.clicked.connect(self.convertButton_callback)  # 一旦按下按钮，连接槽函数进行处理
    ...
    def browserButton_callback(self):
        ...
        if fileName1:
            fileName10 = cv_imread(fileName1)
            self.filePath.setText(fileName1)
            self.showImage(fileName1)
            self.convertButton_callback()
            # print(fileName10)

    def showImage(self,filename = '../temp/temp.png'):
    ...
    # def getOcr_callback(self):
    #     '''
    #     打开截图，点击对勾号会自动保存在目录"../temp/temp.png"
    #     '''
    #     fn = self.screenButton_callback()
    #     txt = getOcr(fn)
    #     print(txt)
    ...
    def screenButton_callback(self):
        ...
        fileName2 = '../temp/temp.png'
        self.cap = ScreenShot()
        self.cap.trigger.connect(self.showImage)
        self.cap.trigger.connect(self.convertButton_callback)
        self.cap.show()
    ...
    def convertButton_callback(self):
        ...
        start = time.time()
        # if self.status:
        #     res = aipOcr.webImage(getImageBytes(fileName1))A
        # else:
        ...
        if self.status == 2:
            text = getOcr(fileName10)
        else:
            text = getOcr(fileName2)
